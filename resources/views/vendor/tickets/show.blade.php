...
@php
    ...
    $showCommunicationStep = $ticket->status === 'waiting_response' || $hasAdditionalInfos;
    $communicationDone = $hasAdditionalInfos;
    $latestReassignRequest = $ticket->latestReassignRequest;
    $reassignPending = $latestReassignRequest && $latestReassignRequest->status === 'pending';
        ...
        <div>
                ...
                <div class="vtd-card-body">
                    ...
                    <div>
                        ...
                        <div class="vtd-attach-grid">
                            ...
                            <a href="{{ route('attachments.ticket.view', $a->id) }}" target="_blank" rel="noopener" class="vtd-attach-item">
                    ...
                    <div class="vtd-addinfo-list">
                        ...
                        <div class="vtd-addinfo-item">
                            ...
                            @if($info->photo_path)
                            <a href="{{ route('attachments.additional-info.view', $info->id) }}" target="_blank" rel="noopener">
                    ...
                    <div class="vtd-addinfo-list">
                        <div class="vtd-attach-label" style="margin-top:.25rem;">Bukti Penyelesaian ke Admin</div>
                        <div class="vtd-addinfo-item" style="background:#f0fdf4;border-color:#bbf7d0;">
                            @if($ticket->completion_note)
                            <p style="color:#166534;">{{ $ticket->completion_note }}</p>
                            @endif
                            @if($ticket->completion_photo_path)
                            <a href="{{ route('attachments.completion-proof.view', $ticket->id) }}" target="_blank" rel="noopener">
            ...
            <div class="vtd-card" style="overflow:visible;">
                <div class="vtd-qa-header">
                    <h6><i class='bx bx-git-branch'></i>Progress Penanganan Tiket</h6>
                ...
                <div class="vtd-card-body">
                    <div class="vtd-checklist">
                        ...
                        <div class="vtd-chk-item {{ $stepDone('in_progress') ? 'done' : 'pending' }}">
                            ...
                            <div><div class="vtd-chk-title">Mulai Kerjakan</div><div class="vtd-chk-desc">Saat Anda mulai kerja, status akan otomatis menjadi "Diproses".</div></div>
                        </div>
                        @if($showCommunicationStep)
                        <div class="vtd-chk-item {{ $communicationDone ? 'done' : 'pending' }}">
                            <div class="vtd-chk-dot">{{ $communicationDone ? '' : '3' }}{!! $communicationDone ? '<i class="bx bx-check"></i>' : '' !!}</div>
                            <div>
                                <div class="vtd-chk-title">Komunikasi ke User</div>
                                <div class="vtd-chk-desc">{{ $communicationDone ? 'Informasi tambahan dari klien sudah diterima.' : 'Vendor sedang menunggu informasi tambahan dari klien.' }}</div>
                            </div>
                        </div>
                        @endif
                        <div class="vtd-chk-item {{ $stepDone('resolved') ? 'done' : 'pending' }}">
                            ...
                            <div><div class="vtd-chk-title">Lapor Penyelesaian</div><div class="vtd-chk-desc">Kirim laporan penyelesaian, status otomatis berubah menjadi Selesai.</div></div>
                        </div>
            ...
            <div class="vtd-card" style="margin-top:1rem;">
                <div class="vtd-card-header">
                    <h6><i class='bx bx-transfer-alt'></i>Ajukan Penugasan Ulang ke Admin</h6>
                </div>
                <div class="vtd-card-body">
                    @if($reassignPending)
                        <div class="vtd-reassign-pending">
                            <i class='bx bx-time-five'></i>
                            <div>
                                <strong>Menunggu Persetujuan Admin</strong>
                                <p>Permintaan penugasan ulang sudah dikirim. Tunggu keputusan admin.</p>
                            </div>
                        </div>
                    @else
                    <form method="POST" action="{{ route('vendor.tickets.request-reassign', $ticket->id) }}" class="vtd-reassign-form">
                        @csrf
                        <label>Alasan Penugasan Ulang</label>
                        <select name="reason_option" required>
                            <option value="">-- Pilih alasan --</option>
                            <option value="beban_tinggi">Beban kerja vendor sedang tinggi</option>
                            <option value="di_luar_spesialisasi">Di luar spesialisasi vendor</option>
                            <option value="lokasi_tidak_terjangkau">Lokasi tidak terjangkau</option>
                            <option value="jadwal_bentrok">Jadwal penanganan bentrok</option>
                            <option value="butuh_peralatan_khusus">Butuh peralatan khusus yang belum tersedia</option>
                            <option value="lainnya">Lainnya</option>
                        </select>
                        <label>Detail Alasan (wajib)</label>
                        <textarea name="reason_detail" rows="3" required placeholder="Jelaskan alasan secara lengkap minimal 10 karakter"></textarea>
                        <button type="submit" class="vtd-btn-confirm"><i class='bx bx-send'></i> Ajukan Penugasan Ulang</button>
                    </form>
                    @endif
                </div>
            </div>
...
<div id="reportModal" class="vtd-modal-ov">
    <div class="vtd-modal-box">
        ...
        <div class="vtd-modal-footer">
            ...
            <form method="POST" action="{{ route('vendor.tickets.update-status', $ticket->id) }}" class="vtd-report-form" enctype="multipart/form-data">
                @csrf @method('PATCH')
                <input type="hidden" name="status" value="resolved">
                <textarea name="completion_note" rows="3" class="form-select" placeholder="Catatan penyelesaian untuk admin (opsional)"></textarea>
                <input type="file" name="completion_photo" class="form-select" accept="image/jpeg,image/png,image/jpg,image/webp">
...
.vtd-reassign-form label {
    font-size: .78rem;
    font-weight: 700;
    color: #64748b;
}
.vtd-reassign-form select,
.vtd-reassign-form textarea {
    width: 100%;
...
.vtd-reassign-form select:focus,
.vtd-reassign-form textarea:focus {
    border-color: #818cf8;
    box-shadow: 0 0 0 3px rgba(99,102,241,.14);
}
.vtd-reassign-pending {
    display: flex;
    gap: .65rem;
    align-items: flex-start;
...
.vtd-reassign-pending i {
    font-size: 1.1rem;
    color: #b45309;
    margin-top: .1rem;
}
.vtd-reassign-pending strong {
    display: block;
    font-size: .84rem;
    color: #92400e;
}
.vtd-reassign-pending p {
    margin: .15rem 0 0;
    font-size: .79rem;
    color: #92400e;
}
...
.vtd-report-form textarea,
.vtd-report-form input[type="file"] {
    width: 100%;
    border: 1px solid #d1d5db;
    border-radius: 10px;
    padding: .55rem .7rem;
